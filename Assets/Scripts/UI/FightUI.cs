...
public class FightUI : MonoBehaviour
{
    ...
    private void MakeMove(MoveMessage message)
    {
        ...
        else if (GlobalManager.singleton.mode == GameMode.Online)
        {
            ...
            cardSlot.PlayAnimation(message.playerIndex + 5 == playerBottom.player.playerID, message.playCard && message.cardPlayed, message.playerIndex >= 0 && message.playCard);
        }
        else
        {
            manager.timeToMakeMove = 0f;

            cardSlot.PlayAnimation(message.playerIndex < 0, message.playCard && message.cardPlayed, message.playerIndex >= 0 && message.playCard);
        }
    }
